function [gamma,x_ridge,solve_Z,epsilon]=DRT_lasso_lan_RL_n_dif(impdata,num,n)
...
Are(:,2) = 1;
Are=[( 2*pi*freq(:) ).^n(1) zeros(size(freq')) Are];
Aim(:,1) = 2*pi*freq(:);
Aim=[zeros(size(freq')) ( 2*pi*freq(:) ).^n(2) Aim];
%计算M矩阵
M_temp=eye(length(freq),length(freq));
M=zeros(length(freq)+4,length(freq)+4);
%% 权重设置
z = abs(Zre+1j*Zim);
weight0 = 1./z;
weight = weight0 ./(mean(weight0));
index = find(freq<1000);
...
options = optimset('algorithm','interior-point-convex','Display','off','TolFun',1e-15,'TolX',1e-15,'MaxFunEvals', 1E5);
lb = 0*ones(numel(freq_M)+4,1);
ub = Inf*ones(numel(freq_M)+4,1);
x_0 = -ones(size(lb));
...
x_ridge_modify(1)=0;x_ridge_modify(2)=0;x_ridge_modify(3)=0;
mu_Z_re2 = Are*x_ridge_modify;
mu_Z_im2 = Aim*x_ridge_modify;

solve_Z=[mu_Z_re,mu_Z_im,mu_Z_re2,mu_Z_im2];
%下舍上入
taumax = ceil(max(log10(1./freq_M)))+1;
taumin = floor(min(log10(1./freq_M)))-1;
freq_fine = logspace(-taumin, -taumax, 20*numel(freq_M));
[gamma_ridge_fine,freq_fine] = map_array_to_gamma(freq_fine, freq_M, x_ridge(5:end), epsilon);
...
if num==0
    ...
    plot(mu_Z_re2,-mu_Z_im2,'og', 'LineWidth', 3);hold on
    plot(Zre,-Zim,'*r', 'LineWidth', 3);
    axis equal
    set(gca,'Fontsize',20,'TickLabelInterpreter','latex');
    ylabel("-Imag/\Omega");xlabel("Real/\Omega");
    legend('计算结果','计算数据-消除电感','实验数据','Location','north');
